...
{
    protected $fillable = [
        'media', 'title', 'slug', 'thumbnail', 'images', 'summary', 'data', 'post_type', 'post_status', 'post_view', 'status', 'published_at', 'author','created_id', 'updated_id', '	deleted_id'
    ];
     ...
     * @return $this
     */
    public static function getMyNews($user_id){
        $listCate = self::getCatePermission($user_id);
        if(empty($listCate)){
            $listCate = "-1";
        }
        return NewsPost::with('cat')
            ->where('news_posts.post_status', '!=', self::STATUS_DELETED)
            ->whereRaw("(news_posts.created_id = {$user_id} OR (news_posts.id IN (SELECT post_id FROM news_category_posts WHERE category_id IN ({$listCate}))))");
