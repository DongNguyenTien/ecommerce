...
class CMB2_Types {
	...
	public function concat_attrs( $attrs, $attr_exclude = array() ) {
		$attributes = '';
		if( $this->field->args( 'dependency' ) ){
			$arg = $this->field->args( 'dependency' );
	...
	public function concat_items( $args = array() ) {
		...
		foreach ( $options as $opt_value => $opt_label ) {
			...
			if( is_array( $opt_label ) ){
				$a['label'] = $opt_label['label'];
				$a['attr']  = $opt_label['attr'];

				$method = 'select_option_with_attr';
			}else{
				$a['label'] = $opt_label;
			}
	 ...
	 * @return string       Generated option element html
	 */
	public function select_option_with_attr( $args = array() ) {
		if( isset( $args['attr'] ) ){
			return sprintf( "\t" . '<option value="%s" %s data-attr="%s">%s</option>', $args['value'], selected( isset( $args['checked'] ) && $args['checked'], true, false ), esc_attr( $args['attr'] ), $args['label'] ) . "\n";
		}else{
			return sprintf( "\t" . '<option value="%s" %s>%s</option>', $args['value'], selected( isset( $args['checked'] ) && $args['checked'], true, false ), $args['label'] ) . "\n";
		}
	...
	/**
	 * Generates html for an group element
	 * @since  1.1.0
	 * @param  $label  $label string label
	 * @param  $option  $option string $option
	 * @return string       Generated option element html
	 */
	public function select_group( $label, $option ) {
		return sprintf( "\t" . '<optgroup label="%s">%s</optgroup>', $label, $option ) . "\n";
	}
	...
	/**
	 * Generate field html tag
	 * @author Kutethemes
	 * @since  1.0
	 * @return string html tag string
	 */
	public function _tag() {
		return $this->field->args( 'tag' );
	}
	/**
	...
	/**
	 * Handles outputting an 'image' element
	 * @author Kutethemes
	 * @since  1.1.0
	 * @param  array  $args Override arguments
	 * @return string       Form input element
	...
	public function image( $args = array() ) {
		$a = $this->parse_args( $args, 'img', array(
			'class' => 'regular-image',
			'name'  => $this->_name(),
			'id'    => $this->_id(),
			'src' 	=> $this->field->escaped_value(),
			'desc'  => $this->_desc( true ),
		) );

		return sprintf( '<img %s alt="%s" />', $this->concat_attrs( $a, array( 'desc' ) ), $a['desc'] );
	}
	/**
	 * Handles outputting an html tag element
	 * @author Kutethemes
	 * @since  1.1.0
	 * @param  array  $args Override arguments
	 * @return string       Form html tag element
	 */

	public function html_tag( $args = array() ) {
		$a = $this->parse_args( $args, $this->_tag(), array(
			'class' => 'tag_field',
			'name'  => $this->_name(),
			'id'    => $this->_id(),
			'tag'	=> $this->_tag(),
		...
		return sprintf( '<%s %s>%s</%s>', $a['tag'], $this->concat_attrs( $a, array( 'desc' ) ), $a['desc'], $a['tag'] );
	}
	...
	/**
	 * The Library Type will be added more control
	 * @author Kutethemes
	 * @since 1.0
	 * Customize Type
	 */
	...
	/**
	 * Drop-down all the sidebar is exist
	 * @author Kutethemes
	 * @since 1.0
	 * @return string
	 */
	...
	/**
	 * Drop-down all the menus were created
	 * @author Kutethemes
	 * @since 1.0
	 * @return string
	 */
	public function menu_select() {
		...
		$option_none  = $this->field->args( 'show_option_none' );
		if ( ! empty( $option_none ) ) {
			$option_none_value = apply_filters( 'cmb2_menu_select_default_value', '' );
			$option_none_value = apply_filters( "cmb2_menu_select_{$this->_id()}_default_value", $option_none_value );
		...
		if ( $menus && !is_wp_error( $menus ) ) :
			foreach ( $menus as $menu ) {
				$options .= $this->select_option( array(
					'label'   => esc_attr( $menu->name ),
					'value'   => esc_attr( $menu->slug ),
					'checked' => $saved_term == esc_attr( $menu->slug ),
				) );
			}
		endif;
		return $this->select( array( 'options' => $options ) );
	...
	/**
	 * Drop-down the google fonts is exist in the plugin
	 * @author Kutethemes
	 * @since 1.0
	 * @return string
	 */
	public function font_select(){
		$fonts = array();
		...
		$options = $system = $all = $custom = '';
		$option_none  = $this->field->args( 'show_option_none' );
		if ( ! empty( $option_none ) ) {
			$option_none_value = apply_filters( 'cmb2_font_select_default_value', '' );
			$option_none_value = apply_filters( "cmb2_font_select_{$this->id()}_default_value", $option_none_value );
		...
		if( key_exists( 'custom', $fonts ) ){
			...
			foreach ( $fonts['custom'] as $font ) {
				$custom .= $this->select_option( array(
					'label'   => str_replace( '#', '', $font),
					'value'   => $font,
					'checked' => $saved_term == $font,
				) );
			}
			$custom = $this->select_group(  __('Custom (uploaded below)','kutetheme'), $custom );
			$options .= $custom;
		}

		foreach ( $fonts['system'] as $font ) {
			$system .= $this->select_option( array(
				'label'   => $font,
				'value'   => $font,
				'checked' => $saved_term == $font,
			) );
		}
		$system = $this->select_group(  __('System','kutetheme'), $system );

		foreach ( $fonts['all'] as $font ) {
			$all .= $this->select_option( array(
				'label'   => $font,
				'value'   => $font,
				'checked' => $saved_term == $font,
			) );
		}
		$all = $this->select_group( __('Google Fonts','kutetheme'), $all );

		return $this->select( array( 'options' => $options  . $system . $all ) );
	}

	/**
	 * Drop-down all the pages in
	 * @author Kutethemes
	 * @since 1.0
	 * @return string
	 *
	...
	/**
	 * Drop-down all the posts were created by post type | Default post
	 * @author Kutethemes
	 * @version 1.0
	 * @return string
	 *
	...
	public function post_select() {
		...
		$tax_query = $this->field->args( 'tax_query' );
		if( $post_type ){
		...
		foreach ( $query as $post ) : setup_postdata( $post ); $id = $post->ID;
			$options .= '<option ' . selected( $saved_term, $id, false ) . ' value="' . $id . '">' . ( get_the_title( $id ) ? get_the_title( $id ) : $id ) . '</option>';
		endforeach;
		return $this->select( array( 'options' => $options ) );
	}
	...
	public function select2( $args = array() ) {
		$a = $this->parse_args( $args, 'select', array(
			'class'   => 'cmb2_select multi-select select2',
			'name'    => $this->_name(),
			'id'      => $this->_id(),
			'desc'    => $this->_desc( true ),
			'options' => $this->concat_items(),
		...
		wp_enqueue_script( 'select2-js', cmb2_utils()->url( "js/select2/select2.min.js" ), array( 'jquery-ui-sortable' ), '2.2.1' );
		wp_enqueue_style( 'select2-css', cmb2_utils()->url( "js/select2/select2.css"), array(), '2.2.1' );
