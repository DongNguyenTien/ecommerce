...
<div class="widget_shopping_cart_content">
    ...
    <?php elseif( $kt_used_header == 2 ):?>
        <div class="col-xs-5 col-sm-4 col-md-2 group-button-header">
            ...
            <?php if( function_exists( 'YITH_WCWL' ) ):
                ...
                <a class="btn-heart" href="<?php echo esc_url( $wishlist_url ); ?>"><?php esc_html_e( 'Yêu thích', 'kutetheme') ?></a>
            <?php endif; ?>
    ...
    <?php elseif( $kt_used_header == 3 ) :?>
        <div class="col-sm-6 col-md-6 col-lg-3 group-button-header">
                <div class="btn-cart" id="cart-block">
                    <a title="<?php esc_html_e( 'My cart', 'kutetheme' ) ?>" href="<?php echo esc_url( $cart_url );?>"><?php _e( 'Giỏ hàng', 'kutetheme');?></a>
                    <span class="notify notify-right"><?php echo esc_attr( $cart_count ); ?></span>
                    <?php do_action( 'kt_mini_cart_content', esc_url( $check_out_url ) ); ?>
                </div>
                ...
                <?php
                if( is_user_logged_in() ) _e( 'Tài khoản', 'kutetheme' );
                else _e( 'Đăng nhập', 'kutetheme' );
                ?>
                </a>
                <?php if( function_exists( 'YITH_WCWL' ) ):
                    $wishlist_url = YITH_WCWL()->get_wishlist_url(); ?>
                    <a title="<?php esc_html_e( "Yêu thích", 'kutetheme') ?>" class="btn-heart" href="<?php echo esc_url( $wishlist_url ); ?>"><?php esc_html_e( 'Yêu thích', 'kutetheme') ?></a>
                <?php endif; ?>
            </div>
    <?php elseif( $kt_used_header == 4 ) :?>
            <div class="col-sm-2 col-md-2 col-lg-1 group-button-header">
                <div class="btn-cart" id="cart-block">
                    <a title="<?php esc_html_e( 'My cart', 'kutetheme' ) ?>" href="<?php echo esc_url( $cart_url );?>"><?php esc_html_e( 'Giỏ hàng', 'kutetheme' ) ?></a>
                    <span class="notify notify-right"><?php echo esc_attr( $cart_count ); ?></span>
                    <?php do_action( 'kt_mini_cart_content', esc_url( $check_out_url ) ); ?>
                </div>
            </div>
    ...
    <?php elseif( $kt_used_header == 6 ) :?>
        <div class="col-xs-5 col-sm-4 col-md-2 group-button-header">
            ...
            <?php endif; ?>
            <div class="btn-cart" id="cart-block">
                <a class="cart-link" title="<?php esc_html_e( 'My cart', 'kutetheme' ) ?>" href="<?php echo esc_url( $cart_url );?>"><?php esc_html_e( 'Giỏ hàng', 'kutetheme' );?></a>
                <span class="notify notify-right"><?php echo esc_attr( $cart_count ); ?></span>
                <?php do_action( 'kt_mini_cart_content', esc_url( $check_out_url ) ); ?>
            </div>
